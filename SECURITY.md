...
No credential is stored in this repository. The upstream API key lives only as a Lambda environment variable. Deployments authenticate through GitHub OIDC against a role restricted to the `production` environment and the `main` branch, so no long-lived AWS key exists in GitHub either.
