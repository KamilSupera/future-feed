...
describe("clientKey", () => {
  ...
  it("ignores CF-Connecting-IP unless the Cloudflare target opts in", () => {
    assert.equal(clientKey(req({ "cf-connecting-ip": "9.9.9.9" }), {}), SHARED_KEY);
  });

  it("trusts CF-Connecting-IP when opted in", () => {
    assert.equal(
      clientKey(req({ "cf-connecting-ip": "9.9.9.9" }), { TRUST_CF_IP: "1" }),
      "9.9.9.9",
    );
  });
